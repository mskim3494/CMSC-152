...
rgb sup(v3 a, v3 b)
{
    double x= clamp(sin(b.x*rand()));
    double y= clamp(sin(b.y*rand()));
    double z= clamp(sin(b.z*rand()));
    return rgb_expr(x,y,z);
}

rgb dist(v3 a, v3 b)
{
    double x= clamp(sin(fabs(b.x-a.x)));
    double y= clamp(sin(fabs(b.y-a.y)));
    double z= clamp(sin(fabs(b.z-a.z)));
    return rgb_expr(x,y,z);
}
...
stage test_stage_7()
{
    ...
    sphere sph1= sphere_expr(v3_expr(0,0.3,0),0.4, &dist, rgb_expr(0.5,0.5,0.0));
    cylinder cyl1= cylinder_expr(v3_expr(0,-3,0.0),.05,6,
                                      &dist, rgb_expr(0.0,0.0,0.0));
